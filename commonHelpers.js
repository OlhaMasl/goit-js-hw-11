...
</li>`).join("")}const m=document.querySelector(".form"),f=document.querySelector(".gallery"),n=document.querySelector(".loader"),p=new c(".gallery a",{captionsData:"alt",captionPosition:"bottom",captionDelay:250});m.addEventListener("submit",s=>{s.preventDefault();const t=s.target.elements.search.value.trim();t!==""&&(s.target.reset(),n.classList.remove("is-hiding"),u(t).then(i=>{i.total===0&&l.show({message:"Sorry, there are no images matching your search query. Please try again!",messageColor:"white",backgroundColor:"red",position:"topRight"});const o=d(i.hits);f.innerHTML=o,p.refresh()}).catch(i=>{l.show({message:"Internal server error!",messageColor:"white",backgroundColor:"red",position:"topRight"})}).finally(()=>{n.classList.add("is-hiding")}))});
